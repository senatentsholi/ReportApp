  ...
  return (
    <ScreenContainer>
      ...
      <GlassCard>
        ...
        <TextInput mode="outlined" label="PRL Name" value={form.principalLecturerName} onChangeText={(value) => setForm((current) => ({ ...current, principalLecturerName: value }))} style={{ marginTop: 12 }} theme={inputTheme} />
        <TextInput
          mode="outlined"
          label="Class Names (comma separated)"
          value={form.classNames.join(', ')}
          onChangeText={(value) => setForm((current) => ({ ...current, classNames: value.split(',') }))}
          style={{ marginTop: 12 }}
          theme={inputTheme}
        />
