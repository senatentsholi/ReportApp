...
export function StudentHomeScreen() {
  ...
  return (
    <ScreenContainer refreshing={refreshing} onRefresh={onRefresh}>
      ...
      <SectionTitle title="Quick Access" caption="Open your extra student tools from here" />
      <View style={{ flexDirection: 'row', gap: 12 }}>
        <StatCard compact label="Monitoring" value={String(data.monitoring.filter((item) => item.className === user.className || item.streamName === user.streamName).length)} onPress={() => navigation.navigate('Monitoring')} />
        <StatCard compact label="Ratings" value={String(visibleRatings.length)} onPress={() => navigation.navigate('Ratings')} colors={['#FFF5E9', '#FFE7D1']} />
      </View>
